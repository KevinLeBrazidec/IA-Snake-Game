...
class App extends Component {
  ...
  isCloser = () => {
    let a = getSnakeHead(this.state.snake).x - this.state.snack.coordinate.x;
    let b = getSnakeHead(this.state.snake).y - this.state.snack.coordinate.y;

    let result = Math.sqrt( a*a + b*b );

    if(this.state.lastDistance != null && result < this.state.lastDistance){

      this.setState({lastDistance: result});
      return true
    }
  ...
  onTick = () => {
    ...
    let objTraining = {"input": this.state.configuration, "output":{key:1}};

    if(this.state.configuration !== null && this.state.score > this.state.lastScore){
      this.state.trainTab.push(objTraining);
      agent.trainingSession(this.state.trainTab);
